...
abstract class FailedJobs implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\FailedJobsTableMap';
    ...
    /**
     * Initializes internal state of Base\FailedJobs object.
     * @see applyDefaults()
     */
    ...
    public function getFailedAt($format = 'Y-m-d H:i:s')
    {
        if ($format === null) {
            ...
            return $this->failed_at instanceof \DateTimeInterface ? $this->failed_at->format($format) : null;
        }
    }
     ...
     * @return $this|\FailedJobs The current object (for fluent API support)
     */
    ...
    public function setException($v)
    {
        ...
        if ($this->exception !== $v) {
            $this->exception = $v;
            $this->modifiedColumns[FailedJobsTableMap::COL_EXCEPTION] = true;
        }
    ...
    public function setFailedAt($v)
    {
        ...
        if ($this->failed_at !== null || $dt !== null) {
            if ($this->failed_at === null || $dt === null || $dt->format("Y-m-d H:i:s.u") !== $this->failed_at->format("Y-m-d H:i:s.u")) {
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : FailedJobsTableMap::translateFieldName('Exception', TableMap::TYPE_PHPNAME, $indexType)];
            $this->exception = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : FailedJobsTableMap::translateFieldName('FailedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 6; // 6 = FailedJobsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\FailedJobs'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(FailedJobsTableMap::COL_EXCEPTION)) {
            $modifiedColumns[':p' . $index++]  = 'exception';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'payload':
                        ...
                        break;
                    case 'exception':
                        $stmt->bindValue($identifier, $this->exception, PDO::PARAM_STR);
                        break;
                    case 'failed_at':
                        $stmt->bindValue($identifier, $this->failed_at ? $this->failed_at->format("Y-m-d H:i:s.u") : null, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(FailedJobsTableMap::COL_EXCEPTION)) {
            $criteria->add(FailedJobsTableMap::COL_EXCEPTION, $this->exception);
        }
        if ($this->isColumnModified(FailedJobsTableMap::COL_FAILED_AT)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setException($this->getException());
